...
PACKAGENAME = metadata.get('package_name', 'nuclockutils')
DESCRIPTION = metadata.get('description', 'Utilities for a precise timing calibration of NuSTAR data')
AUTHOR = metadata.get('author', 'Matteo Bachetti')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'http://github.com/matteobachetti/nustar-clock-utils')
__minimum_python_version__ = metadata.get("minimum_python_version", "3.6")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: nuclockutils requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
